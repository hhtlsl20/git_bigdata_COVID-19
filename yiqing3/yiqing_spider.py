...
def get_data():
    ...
    #美国各州当日详细数据
    detailsusa = []
    for i in range(len(data_all3['foreignList'])):
        continent = data_all3['foreignList'][i]["continent"]
        name = data_all3['foreignList'][i]["name"]
        if 'children' in data_all3['foreignList'][i].keys():
            data_province = data_all3['foreignList'][i]["children"]
            for city_infos in data_province:
                ds = "2020." + city_infos["date"]
                tup = time.strptime(ds, "%Y.%m.%d")  # 匹配时间
                ...
                detailsusa.append([update_time, continent, name, city, namemap, confirm, confirm_add, heal, dead])
        else:
            pass
    #
...
def get_conn():
	#建立连接
	conn = pymysql.connect(host="8.129.42.178", user="hht", password="hhtLSL520!", db="yq", charset="utf8mb4")
	#创建游标
	cursor = conn.cursor()
	return conn,cursor
...
def update_detailsusa():
	...
	try:   
		...
		sql = "insert into detailsusa(update_time, continent, name, city, namemap, confirm, confirm_add, heal, dead) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
